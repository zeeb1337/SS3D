...
namespace SS3D.Engine.Atmospherics
{
    public class PipeObject : PipeGeneric
    {
        ...
        private float[] difference = new float[AtmosGas.GasesCount];
        ...
        public void CalculateFlux()
        {
            ...
            foreach (PipeObject pipe in AtmosNeighbours)
            {
                ...
                {
                    neighbourFlux[i] = Mathf.Min(tileFlux[i] * AtmosGas.FluidDrag + (pressure - pipe.GetPressure()) * AtmosGas.DeltaTime, 1000f);
            ...
            if (neighbourFlux[0] > AtmosGas.FluxEpsilon || neighbourFlux[1] > AtmosGas.FluxEpsilon || neighbourFlux[2] > AtmosGas.FluxEpsilon ||
                neighbourFlux[3] > AtmosGas.FluxEpsilon)
            {
                float scalingFactor = Mathf.Min(1f,
                    pressure / (neighbourFlux[0] + neighbourFlux[1] + neighbourFlux[2] + neighbourFlux[3]) / AtmosGas.DeltaTime);
        ...
        public void SimulateFlux()
        {
            if (state == AtmosStates.Active)
            {
                ...
                foreach (PipeObject pipe in AtmosNeighbours)
                {
                    ...
                    {
                        difference = (atmosContainer.GetTemperature() - pipe.atmosContainer.GetTemperature()) * AtmosGas.ThermalBase * atmosContainer.Volume;
        ...
        public void SimulateMixing()
        {
            ...
            if (AtmosHelper.ArrayZero(atmosContainer.GetGasses(), AtmosGas.MixRate))
                return;
            ...
            for (int i = 0; i < AtmosGas.GasesCount; i++)
            {
                ...
                if (atmosContainer.GetGasses()[i] > 0f)
                {
                    ...
                    foreach (PipeObject atmosObject in AtmosNeighbours)
                    {
                        ...
                        {
                            difference[i] = (atmosContainer.GetGasses()[i] - atmosObject.GetAtmosContainer().GetGasses()[i]) * AtmosGas.MixRate;
                            if (difference[i] > 0.01f)
                            {
                                // Increase neighbouring tiles moles
                                atmosObject.GetAtmosContainer().AddGas(i, difference[i]);
                                if (Mathf.Abs(atmosObject.GetPressure() - GetPressure()) > AtmosGas.MixRate)
                                {
