...
namespace SS3D.Core.Tilemaps.Connections
{
    /// <summary>
    ...
    public class MultiAdjacencyConnector : NetworkBehaviour, IAdjacencyConnector
    {
        ...
        public byte BlockedConnections => _blockedConnections;
        ...
        {
            EnsureInit();
            _blockedConnections = newConnections;
            UpdateMeshAndDirection();
        }
        ...
        public override void OnStartClient()
        {
            ...
            SyncBlockedConnections(_blockedConnections, EditorBlockedConnections);
            base.OnStartClient();
        }
        ...
        {
            TilemapData map = GetComponentInParent<TilemapData>();
            ...
            foreach (TileLayer layer in TileHelper.GetTileLayers())
            {
                ...
                if ((neighbour.GetPlacedObject(0) && neighbour.GetPlacedObject(0).HasAdjacencyConnector()) || layer == ownLayer)
                {
                    ...
                    if (connected)
                        neighbour.GetPlacedObject(0)?.UpdateSingleNeighbour(TileHelper.GetOpposite(dir), GetComponent<PlacedTileObject>());
                }
            }
        ...
        private bool UpdateSingleConnection(Direction dir, PlacedTileObject placedObject)
        {
            ...
            if (placedObject)
            {
                ...
                isConnected &= ((_blockedConnections >> (int) dir) & 0x1) == 0;
            }
        ...
        public void SetBlockedDirection(Direction dir, bool value)
        {
            ...
            SyncBlockedConnections(_blockedConnections, _adjacencyMap.SerializeToByte());
        ...
        public void CleanAdjacencies()
        {
            ...
            for (int i = 0; i < neighbourObjects.Length; i++)
            {
                neighbourObjects[i]?.UpdateSingleNeighbour(TileHelper.GetOpposite((Direction)i), null);
            }
            ...
            if (CrossConnectAllowed)
            {
                foreach (TileLayer layer in TileHelper.GetTileLayers())
                {
                    for (int i = 0; i < neighbourObjects.Length; i++)
                    {
                        ...
                        {
                            neighbour.GetPlacedObject(0).UpdateSingleNeighbour(TileHelper.GetOpposite((Direction)i), null);
                        }
                    }
                }
            }
